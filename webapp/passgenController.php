...
if ($allcaps === 'true') {
    $variations['allcaps'] = true;
} elseif ($allcaps === 'false') {
    $variations['allcaps'] = false;
} else {
    $error['allcaps'] = 'invalid value for allcaps';
}
$capitalize = isset ($_POST['capitalize']) ? urldecode($_POST['capitalize']) : (isset($_GET['capitalize']) ? $_GET['capitalize'] : null);
if ($capitalize === 'true') {
    $variations['capitalize'] = true;
} elseif ($capitalize === 'false') {
    $variations['capitalize'] = false;
} else {
    $error['capitalize'] = 'invalid value for capitalize';
}
$punctuate = isset ($_POST['punctuate']) ? urldecode($_POST['punctuate']) : (isset($_GET['punctuate']) ? $_GET['punctuate'] : null);
if ($punctuate === 'true') {
    $variations['punctuate'] = true;
} elseif ($punctuate === 'false') {
    $variations['punctuate'] = false;
} else {
    $error['punctuate'] = 'invalid value for punctuate';
}
$addslashes = isset ($_POST['addslashes']) ? urldecode($_POST['addslashes']) : (isset($_GET['addslashes']) ? $_GET['addslashes'] : null);
if ($addslashes === 'true') {
    $variations['addslashes'] = true;
} elseif ($addslashes === 'false') {
    $variations['addslashes'] = false;
} else {
    $error['addslashes'] = 'invalid value for addslashes';
}

$separators = isset ($_POST['separators']) ? urldecode($_POST['separators']) : (isset($_GET['separators']) ? $_GET['separators'] : null);
if ($separators !== null && (!is_string($separators))) {
    $separators = null;
    $error['separators'] = 'separators must be a string';
}
...
if ($minWordSize !== null && (!is_int($minWordSize) || $minWordSize < 0)) {
    $minWordSize = null;
    $error['minWordSize'] = 'minWordSize must be an integer > 0';
}

$level = isset ($_POST['level']) ? intval(urldecode($_POST['level'])) : (isset($_GET['level']) ? intval($_GET['level']) : null);
if ($level !== null && (!is_int($level) || $level < 0 || $level > 3)) {
    $level = null;
    $error['level'] = 'level must be an integer between 0 and 3';
}
