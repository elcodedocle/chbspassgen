...
class Dictionary implements DictionaryInterface
{
    ...
    public function countWords($words = null, $minWordSize = null, $minReadWordsWordSize = null)
    {
        ...
        if ($minWordSize === null) {
            $minWordSize = $this->minWordSize;
        }
        if ($minReadWordsWordSize === null) {
            $minReadWordsWordSize = $this->minWordSize;
        }
        if (!array($words)) {
    ...
    public function getRandomWordsArray($count = null, $unique = null)
    {
        ...
        if (!is_int($count) || $count < 1 || !is_bool($unique)) {
    ...
    public function readWordsFromArray($newWords, $add = null, $minWordSize = null)
    {
        ...
        if ($minWordSize === null) {
            $minWordSize = $this->defaultMinWordSize;
        }
        if (
            !is_bool($add)
        ) {
        ...
        return $this->readWordsFromArrayInternal($newWords, $add, $minWordSize);
    }
    ...
    public function readWordsFromFile($filename = null, $splitPattern = null, $add = null, $minWordSize = null)
    {
        ...
        if ($splitPattern === null) {
            $splitPattern = $this->defaultSplitPattern;
        }
        if ($add === null) {
            $add = $this->defaultAdd;
        }
        ...
        return $this->readWordsFromArrayInternal($words, $add, $minWordSize);
    }
    ...
    {
        if (!is_int($minWordSize || $minWordSize < 0)) {
            throw new Exception('$minWordSize must be int>=0');
        }
        if ($recount) {
    ...
    public function readWords($input = null, $format = null, $params = array(), $minWordSize = null)
    {
        ...
        if (isset($params['add'])) {
            if (!is_bool($params['add'])) {
                throw new Exception ('$params["add"] must be boolean');
            }
            $add = $params['add'];
        } else {
            $add = $this->defaultAdd;
        ...
        switch ($format) {
            ...
            case 'pdo_handler':
                if ($input === null) {
                    $input = $this->defaultDbh;
                }
                if (!($input instanceof PDO)) {
                    throw new Exception ('$input for this format must be a PDO instance');
                }
    ...
    public function __construct($input = null, $minWordSize = null, $format = null, $params = array(), $prng = null)
    {
        ...
        if ($minWordSize === null) {
            ...
            $this->defaultMinWordSize = $minWordSize;
        }
        if (!is_int($minWordSize) || $minWordSize <= 0) {
            throw new Exception ('$minWordSize must be a positive (>0) integer');
        }
        ...
        if (!is_string($format)) {
            throw new Exception ('$format must be string');
        }

        if (!is_array($params)) {
            throw new Exception ('$params must be array');
        }
        if (isset($params['add']) && is_bool($params['add'])) {
            $this->defaultAdd = $params['add'];
        }
        if (isset($params['splitPattern'])) {
            $this->defaultSplitPattern = $params['splitPattern'];
        }
        if (isset($params['table'])) {
            $this->defaultTable = $params['table'];
        }
        if (isset($params['column'])) {
            $this->defaultColumn = $params['column'];
        }
        ...
        if ($prng === null) {
            ...
            if (!is_object($prng) || !method_exists($prng, 'rand')) {
                // An interface would be more appropriate here
     ...
     * @throws Exception
     */
    private function readWordsFromArrayInternal(array $newWords, $add, $minWordSize)
    {
        foreach ($newWords as $index => $word) {
            if (strlen($newWords[$index] = trim($word)) < $minWordSize) {
                unset($newWords[$index]);
            }
        }
        if ($add) {
