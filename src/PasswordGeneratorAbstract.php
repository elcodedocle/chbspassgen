...
abstract class PasswordGeneratorAbstract
{
    ...
    public function setDictionary($dictionary)
    {
        if (is_object($dictionary) && in_array('synapp\info\tools\passwordgenerator\dictionary\DictionaryInterface', class_implements($dictionary))) {
            ...
            throw new Exception("Unable to set dictionary.");
        }
    }
    ...
    public function calculateEntropy($symbolCount, $alphabetSize = null)
    {
        ...
        if (!is_int($symbolCount) || !is_int($alphabetSize)) {
            throw new Exception("Unable to calculate entropy.");
        }
    ...
    public function getEntropy($password = '', $dictionary = null, $variationsCount = null, $lastOrSeparator = true, $separatorsCount = null)
    {
        ...
        if ($separatorsCount === null) {
            $separatorsCount = ($lastOrSeparator !== false) ? $this->getSeparatorsCount() : 1;
    ...
    public function getMinSymbolCount($minEntropy = null, $dictionary = null, $variationsCount = null)
    {
        ...
        if ($minEntropy === null) {
            $minEntropy = $this->getMinEntropy();
        } elseif (!is_numeric($minEntropy)) {
            throw new Exception ('$minEntropy must be numeric');
        }
        if ($dictionary === null) {
            $dictionary = $this->dictionary;
        ...
        if ($symbolCount > $maxSymbolCount) {
            throw new Exception ('$symbolcount must be > $maxSymbolCount');
        }
        return $symbolCount;
    }
